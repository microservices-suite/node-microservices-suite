...
const thService = require('../services/services');
...
const subscriberHandler = async ({ channel, msg, service, config }) => {
    ...
    switch (action) {
        case 'create':
            try {
                await thService.createTh({ body })
            } catch (error) {
                //TODO: handle this case. Eg do we roll back or what next..
                console.log(`th: subscriber-failed to create th: ${error.message}`);
        ...
        case 'read':
            const { ths } = await thService.getThs();
            data = ths;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'read:id':
            th = await thService.getThById({ id: body.id });
            th = th.th;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'update':
