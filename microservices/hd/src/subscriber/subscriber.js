...
const hdService = require('../services/services');
...
const subscriberHandler = async ({ channel, msg, service, config }) => {
    ...
    switch (action) {
        case 'create':
            try {
                await hdService.createHd({ body })
            } catch (error) {
                //TODO: handle this case. Eg do we roll back or what next..
                console.log(`hd: subscriber-failed to create hd: ${error.message}`);
        ...
        case 'read':
            const { hds } = await hdService.getHds();
            data = hds;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'read:id':
            hd = await hdService.getHdById({ id: body.id });
            hd = hd.hd;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'update':
