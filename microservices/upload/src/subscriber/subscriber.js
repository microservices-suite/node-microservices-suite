...
const uploadService = require('../services/services');
...
const subscriberHandler = async ({ channel, msg, service, config }) => {
    ...
    switch (action) {
        case 'create':
            try {
                await uploadService.createUpload({ body })
            } catch (error) {
                //TODO: handle this case. Eg do we roll back or what next..
                console.log(`upload: subscriber-failed to create upload: ${error.message}`);
        ...
        case 'read':
            ...
            break;
        case 'read:id':
            upload = await uploadService.getUploadById({ id: body.id });
