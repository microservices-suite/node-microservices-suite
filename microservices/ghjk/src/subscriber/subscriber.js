...
const ghjkService = require('../services/services');
...
const subscriberHandler = async ({ channel, msg, service, config }) => {
    ...
    switch (action) {
        case 'create':
            try {
                await ghjkService.createGhjk({ body })
            } catch (error) {
                //TODO: handle this case. Eg do we roll back or what next..
                console.log(`ghjk: subscriber-failed to create ghjk: ${error.message}`);
        ...
        case 'read':
            const { ghjks } = await ghjkService.getGhjks();
            data = ghjks;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'read:id':
            ghjk = await ghjkService.getGhjkById({ id: body.id });
            ghjk = ghjk.ghjk;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'update':
