...
const kjhgfService = require('../services/services');
...
const subscriberHandler = async ({ channel, msg, service, config }) => {
    ...
    switch (action) {
        case 'create':
            try {
                await kjhgfService.createKjhgf({ body })
            } catch (error) {
                //TODO: handle this case. Eg do we roll back or what next..
                console.log(`kjhgf: subscriber-failed to create kjhgf: ${error.message}`);
        ...
        case 'read':
            const { kjhgfs } = await kjhgfService.getKjhgfs();
            data = kjhgfs;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'read:id':
            kjhgf = await kjhgfService.getKjhgfById({ id: body.id });
            kjhgf = kjhgf.kjhgf;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'update':
