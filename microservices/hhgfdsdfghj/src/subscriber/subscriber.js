...
const hhgfdsdfghjService = require('../services/services');
...
const subscriberHandler = async ({ channel, msg, service, config }) => {
    ...
    switch (action) {
        case 'create':
            try {
                await hhgfdsdfghjService.createHhgfdsdfghj({ body })
            } catch (error) {
                //TODO: handle this case. Eg do we roll back or what next..
                console.log(`hhgfdsdfghj: subscriber-failed to create hhgfdsdfghj: ${error.message}`);
        ...
        case 'read':
            const { hhgfdsdfghjs } = await hhgfdsdfghjService.getHhgfdsdfghjs();
            data = hhgfdsdfghjs;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'read:id':
            hhgfdsdfghj = await hhgfdsdfghjService.getHhgfdsdfghjById({ id: body.id });
            hhgfdsdfghj = hhgfdsdfghj.hhgfdsdfghj;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'update':
            break;
