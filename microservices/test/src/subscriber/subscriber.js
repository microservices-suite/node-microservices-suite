...
const testService = require('../services/services');
    ...
    switch (action) {
        case 'create':
            try {
                ...
                console.log(`test: subscriber-failed to create test: ${error.message}`);
        ...
        case 'read':
            ...
            break;
        case 'read:id':
            test = await testService.getTestById({ id: body.id });
            test = test.test;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'update':
