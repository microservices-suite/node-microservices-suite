...
const rbacService = require('../services/services');
...
const subscriberHandler = async ({ channel, msg, service, config }) => {
    ...
    switch (action) {
        case 'create':
            try {
                await rbacService.createRbac({ body })
            } catch (error) {
                //TODO: handle this case. Eg do we roll back or what next..
                console.log(`rbac: subscriber-failed to create rbac: ${error.message}`);
        ...
        case 'read':
            const { rbacs } = await rbacService.getRbacs();
            data = rbacs;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'read:id':
            rbac = await rbacService.getRbacById({ id: body.id });
            rbac = rbac.rbac;
            await rabbitmq.workerQueue.sendToQueue({ data, queue: service, config });
            break;
        case 'update':
