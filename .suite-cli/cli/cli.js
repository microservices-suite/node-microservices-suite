...
const {
  ...
  scaffoldGateways,
  readFileContent,
} = require("./scripts/scripts.module");
...
program
  ...
  .requiredOption(
    "-n, --workspace-name <name>",
    "Name of the workspace where to add dependencies"
  )
  .option(
    "-d, --workspace-directory <directory>",
    'Directory where to look for the workspace. Defaults to "microservices"',
    "microservices"
  )
  .argument("<packages...>", "Space-separated list of packages to add")
  .action(
    async (packages, options) =>
      await actionHandlers.addDepsAtWorkspace({ packages, options })
  );
program
...
program
  .command("prune")
  .description(
    "prunes docker artifacts. Does not prompt for confirmation. See docker [system,volume] prune --help"
  )
...
program
  .command("install")
  .description(
    'Installs dependencies at given workspace. If not specified workspace defaults to "microservices"'
  )
  .requiredOption(
    "-n, --workspace-name <name>",
    "Name of the workspace where to install dependencies"
  )
  .option(
    "-d, --workspace-directory <directory>",
    "Name of the directory where to install dependencies",
    "microservices"
  )
  .argument("<packages...>", "Space-separated list of packages to install")
  .action(
    async (packages, options) =>
      await actionHandlers.installDepsAtWorkspace({ packages, options })
  );
program
  .command("reset [components...]")
  .description(
    "deep remove all node modules and artefacts generated during yarn install"
  )
  .action(
    async (components, options) =>
      await actionHandlers.repoReset({ options, components })
  );
program
  .command("release [package]")
  .description(
    "make a package release and publish simultaneously to npm.If no package is passed then generate:release for changelog is run"
  )
...
program
  ...
  .description(
    "Starts specified components (services or apps), or all services in dev mode if -m is not specified"
  )
  .option(
    "-a, --app",
    "Indicates that the components specified are apps. Defaults to Docker Compose"
  )
  .option(
    "-k, --kubectl",
    "Run the components using kubectl instead of Docker Compose. This flag is ignored if -a is not provided"
  )
  .option(
    "-b, --build",
    "If running dockerized up docker compose rebuilds the images i.e docker compose up --build"
  )
  .option(
    "-v, --vanilla",
    "Run the components directly without Docker. In development mode with Nodemon, otherwise with PM2"
  )
...
program
  ...
  .description(
    "stop running containers. Stops all containers if no app is specified"
  )
  .action(async (components, options) => {
...
program
  ...
  .action(async () => {
    try {
      ...
      if (installYarn) {
        try {
          ...
          spinner.fail(
            "Failed to install Yarn. Please install it manually and try again."
          );
          process.exit(1);
        }
      } else {
    ...
    prompt([
      ...
      try {
        existing_services = getExistingComponent({
          key: "services",
          currentDir: cwd(),
        });
      } catch (error) {}
      switch (answers.resource) {
          ...
          prompt([
            {
              ...
              validate: (input) =>
                input ? true : "Repo name cannot be empty.",
            },
            {
              type: "checkbox",
              name: "apis",
            ...
            },
            {
              ...
              default: 9001,
              validate: (input) =>
                input === "" || !isNaN(input) ? true : "Port must be a number.",
            },
          ]).then((answers) => {
        ...
        case "service":
          prompt([
            ...
            const service_idx = existing_services.findIndex(
              (s) => s.name === service_name
            );
            // if service exists return the port otherwise generate next available port
            const next_port =
              existing_services[service_idx]?.port ||
              getNextAvailablePort({ key: existing_services, port: "port" });
            prompt([
              {
                type: "input",
                name: "port",
        ...
        case "library":
          prompt([
            ...
            actionHandlers.scaffoldNewLibrary({
              answers: { ...answers, private: false },
            })
          );
          break;
        case "app":
          const existing_apps =
            getExistingComponent({ key: "apps", currentDir: cwd() }) || [];
          ...
          prompt([
            ...
            const next_port =
              existing_apps[app_idx]?.GATEWAY_PORT ||
              getNextAvailablePort({
                key: existing_apps,
                port: "GATEWAY_PORT",
              });
            prompt([
              {
                ...
                default: next_port,
                validate: (input) =>
                  !isNaN(input) ? true : "Port must be a number.",
              },
              {
                type: "input",
                name: "api_version",
              ...
              },
              {
                ...
                message:
                  "How long do you want the gateway to cache data (optional):",
                default: 3600,
                validate: (input) =>
                  !isNaN(input) ? true : "Caching period must be a number.",
              },
              {
                type: "input",
                name: "gateway_timeout",
                message:
                  "How long should a request take before timing out (optional):",
                default: 300,
                validate: (input) =>
                  input === "" || !isNaN(input)
                    ? true
        ...
        case "gateway":
          ...
          if (!apps) {
            logInfo({
              message: `No apps found in this project. You need to have atleast one app to generate a gateway`,
            });
...
program
  ...
  .description(
    "Clean remove a monorepo resource or all resources of a specific type with the --all flag."
  )
  ...
  .action(async (resource, resource_name, options) => {
    ...
    if (!["service", "app", "library", "gateway"].includes(resource)) {
      spinner.fail(
        'Supported resources are ["service","app","library","gateway"] '
      );
      return;
    }
    ...
    if (!resource_name && !options.all) {
      spinner.fail(
        "You must provide either a resource name or the --all flag to remove all resources."
      );
      return;
    }
    ...
    try {
      ...
      await actionHandlers.removeResource({
        answers: { resource, resource_name, options },
      });
    } catch (error) {
      spinner.fail(
        `Failed to remove ${resource} ${resource_name || "resources"}: ${
          error.message
        }`
      );
    }
